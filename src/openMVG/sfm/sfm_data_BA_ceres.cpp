	...
	namespace sfm {
			...
			bUse_loss_function_(true)
		{
			...
			if (ceres::IsSparseLinearAlgebraLibraryTypeAvailable(ceres::SUITE_SPARSE))
			{
				...
				// std::cout << "SUITE_SPARSE" << std::endl;
		...
		bool Bundle_Adjustment_Ceres::PartialAdjust
		(
		...
		)
		{
			...
			IndexT partial_bundle_adjustment_num_views = 20;
			std::unordered_set<IndexT> views_to_optimize, views_to_constant;
			std::unordered_set<IndexT> intrinsic_to_optimize, intrinsic_to_to_constant;
			for (std::vector<IndexT>::const_reverse_iterator citer = reconstructed_views_.rbegin();
				citer != reconstructed_views_.rend();
				citer++)
			{
				if (!sfm_data.views[*citer]->is_estimated)
					continue;
				if (views_to_optimize.size() > partial_bundle_adjustment_num_views)
					break;
				else
				{
					views_to_optimize.insert(*citer);
					intrinsic_to_optimize.insert(sfm_data.views[*citer]->id_intrinsic);
				}
			}
			// Get the tracks observed in these views.
			std::unordered_set<IndexT> tracks_to_optimize;
			for (const IndexT view_to_optimize : views_to_optimize)
			{
				const View* view = sfm_data.views[view_to_optimize].get();
				const std::unordered_map<IndexT, IndexT>& tracks = view->features;
				for (const auto& track : tracks) 
				{
					tracks_to_optimize.insert(track.first);
				}
			}
			for (const IndexT trackId : tracks_to_optimize)
			{
				const Landmark& landmark = sfm_data.structure[trackId];
				for (const auto& observation : landmark.obs)
				{
					if (views_to_optimize.count(observation.first) == 0)
					{
						views_to_constant.insert(observation.first);
						intrinsic_to_to_constant.insert(sfm_data.views[observation.first]->id_intrinsic);
						//intrinsic in constant views also should been constanted
						intrinsic_to_optimize.erase(sfm_data.views[observation.first]->id_intrinsic);
					}
				}
			}
			// Data wrapper for refinement:
			...
			// Setup Poses data & subparametrization
			for (const auto & view_id : views_to_optimize)
			{
				const Pose3 & pose = sfm_data.poses[view_id];
				const Mat3 R = pose.rotation();
				const Vec3 t = pose.translation();
				...
				ceres::RotationMatrixToAngleAxis((const double*)R.data(), angleAxis);
				// angleAxis + translation
				map_poses[view_id] = { angleAxis[0], angleAxis[1], angleAxis[2], t(0), t(1), t(2) };

				double * parameter_block = &map_poses[view_id][0];
				problem.AddParameterBlock(parameter_block, 6);
				if (options.extrinsics_opt == Extrinsic_Parameter_Type::NONE)
				{
					// set the whole parameter block as constant for best performance
			...
			for (const auto & view_id : views_to_constant)
			{
				const Pose3 & pose = sfm_data.poses[view_id];
				const Mat3 R = pose.rotation();
				const Vec3 t = pose.translation();
			...
			// Setup Intrinsics data & subparametrization
			for (const auto& intrinsicId : intrinsic_to_optimize)
			{
				auto& intrinsic = sfm_data.intrinsics[intrinsicId];
				if (isValid(intrinsic->getType()))
				{
					map_intrinsics[intrinsicId] = intrinsic->getParams();
					if (!map_intrinsics[intrinsicId].empty())
					{
						double * parameter_block = &map_intrinsics[intrinsicId][0];
						problem.AddParameterBlock(parameter_block, map_intrinsics[intrinsicId].size());
						if (options.intrinsics_opt == Intrinsic_Parameter_Type::NONE)
						{
							// set the whole parameter block as constant for best performance
							problem.SetParameterBlockConstant(parameter_block);
						...
						{
							const std::vector<int> vec_constant_intrinsic =
								intrinsic->subsetParameterization(options.intrinsics_opt);
							if (!vec_constant_intrinsic.empty())
							{
								ceres::SubsetParameterization *subset_parameterization =
									new ceres::SubsetParameterization(
										map_intrinsics[intrinsicId].size(), vec_constant_intrinsic);
								problem.SetParameterization(parameter_block, subset_parameterization);
							}
						}
					}
			...
			for (const auto& intrinsicId : intrinsic_to_to_constant)
			{
				auto& intrinsic = sfm_data.intrinsics[intrinsicId];
				if (isValid(intrinsic->getType()))
				{
			...
			// For all visibility add reprojections errors:
			for (const auto trackId : tracks_to_optimize)
			{
				...
				if (options.structure_opt == Structure_Parameter_Type::NONE)
					problem.SetParameterBlockConstant(landmark.X.data());
			}
			...
			else // Solution is usable
			{
				...
				if (options.extrinsics_opt != Extrinsic_Parameter_Type::NONE)
				{
					for (auto  viw_id : views_to_optimize)
					{
						Mat3 R_refined;
						ceres::AngleAxisToRotationMatrix(&map_poses[viw_id][0], R_refined.data());
						Vec3 t_refined(map_poses[viw_id][3], map_poses[viw_id][4], map_poses[viw_id][5]);
						// Update the pose
						Pose3 & pose = sfm_data.poses[viw_id];
						pose = Pose3(R_refined, -R_refined.transpose() * t_refined);
					}
				}
				...
				if (options.intrinsics_opt != Intrinsic_Parameter_Type::NONE)
				{
					for (auto  intrinsic_id : intrinsic_to_optimize)
					{
						const std::vector<double> & vec_params = map_intrinsics[intrinsic_id];
						sfm_data.intrinsics[intrinsic_id]->updateFromParams(vec_params);
					}
				}
