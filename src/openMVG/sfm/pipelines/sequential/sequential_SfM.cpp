...
namespace openMVG {
	namespace sfm {
		...
		bool SequentialSfMReconstructionEngine::Process() {
			...
			//initial_pair_.first = 142;
			//initial_pair_.second = 167;
			// Initial pair choice
			...
			std::cout << "pair :" << initial_pair_.first << "," << initial_pair_.second << std::endl;
			...
			double full_bundle_adjustment_growth_percent = 0.05;
			...
			while (FindImagesWithPossibleResection(vec_possible_resection_indexes))
			{
				...
				bool bImageAdded = Resection(vec_possible_resection_indexes[0]);
				set_remaining_view_id_.erase(vec_possible_resection_indexes[0]);
				if (bImageAdded)
				{
					auto view_to_remove = std::find(reconstructed_views_.begin(),
						reconstructed_views_.end(), vec_possible_resection_indexes[0]);
					if (view_to_remove != reconstructed_views_.end())
						reconstructed_views_.erase(view_to_remove);

					reconstructed_views_.push_back(vec_possible_resection_indexes[0]);
				  ...
				  /*  std::ostringstream os;
					...
					bool runPartialBundleAdjustment = (reconstructed_views_.size() - num_optimized_views_) / static_cast<double>(sfm_data_.views.size()) < full_bundle_adjustment_growth_percent;
					if (runPartialBundleAdjustment)
					{
					...
					//update reconstructed_views_
					for (std::vector<IndexT>::iterator iter = reconstructed_views_.begin();
						iter != reconstructed_views_.end();)
					{
						if (sfm_data_.views[*iter]->is_estimated)
							++iter;
						else
						{
							iter = reconstructed_views_.erase(iter);
							set_remaining_view_id_.insert(*iter);
							if (!runPartialBundleAdjustment)
...
#pragma omp critical
		...
		bool SequentialSfMReconstructionEngine::MakeInitialPair3D(const Pair & current_pair)
		{
			...
			if (bRefine_using_BA)
			{
				...
				reconstructed_views_.reserve(sfm_data_.views.size());
				reconstructed_views_.push_back(view_I->id_view);
				reconstructed_views_.push_back(view_J->id_view);
				sfm_data_.views[view_I->id_view]->is_estimated = true;
				sfm_data_.views[view_J->id_view]->is_estimated = true;
					...
					iter != tiny_scene.GetLandmarks().end(); ++iter)
				{
						...
						residual_J.norm() < relativePose_info.found_residual_precision)
					{
						...
						sfm_data_.views[view_I->id_view]->features[trackId] = ob_xI.id_feat;
						sfm_data_.views[view_J->id_view]->features[trackId] = ob_xJ.id_feat;
					}
				}
			...
			std::vector<uint32_t> & vec_possible_indexes)
		{
			...
			std::vector<std::unordered_map<int, int> > tracksInImage(num_thread);
#ifdef OPENMVG_USE_OPENMP
#pragma omp parallel
#endif
			{
				for (const auto& track : sfm_data_.structure)
				{
					...
					const submapTrack& _submapTrack = map_tracks_[track.first];
					for (const auto& observation : _submapTrack)
					{
						if (set_remaining_view_id_.count(observation.first) == 1)
						{
							std::unordered_map<int, int>& v = tracksInImage[thread_num];
							if (v.count(observation.first) == 1)
								v[observation.first] += 1;
							else
								v[observation.first] = 1;
						}
					}
			...
			std::unordered_map<int, int> image_tracksMap;
			for (int i = 0; i < num_thread; i++)
			{
				std::unordered_map<int, int>& v = tracksInImage[i];
				for (const auto& pair : v)
				{
					if (image_tracksMap.count(pair.first) == 1)
						image_tracksMap[pair.first] += pair.second;
					else
						image_tracksMap[pair.first] = pair.second;
				}
			}
			if (image_tracksMap.empty())
				return false;
			std::pair<int, int> max_tracks_in_image(-1, -1);// view_id,num_tracks
			for (std::unordered_map<int, int>::const_iterator citer = image_tracksMap.begin();
				citer != image_tracksMap.end();
				++citer)
			{
				if (citer->second > max_tracks_in_image.second)
				{
					max_tracks_in_image.first = citer->first;
					max_tracks_in_image.second = citer->second;
				}
			}
			vec_possible_indexes.push_back(max_tracks_in_image.first);
			std::cout << "FindImagesWithPossibleResection Task done in (ms): " << timer.elapsedMs() << std::endl;
			return true;
			//			system::Timer timer;
			...
			//			//}
			//			//only return the view that share the most of 2D-3D correspondences
			//			std::cout << "FindImagesWithPossibleResection Task done in (ms): " << timer.elapsedMs() << std::endl;
			//			return true;
		}
		...
		bool SequentialSfMReconstructionEngine::Resection(const uint32_t viewIndex)
		{
			...
			TracksUtilsMap::GetTracksInImage(viewIndex, map_tracks_, map_tracksCommon);
			std::set<uint32_t> set_tracksIds;
			...
			//    - If the track does not exists, try robust triangulation & add the new valid view track observation
			{
				...
				for (const std::pair< uint32_t, tracks::submapTrack >& trackIt : map_tracksCommon)
				{
						...
						)
					{
						...
						{
							View * view_J = sfm_data_.GetViews().at(J).get();
							const IntrinsicBase * cam_J = sfm_data_.GetIntrinsics().at(view_J->id_intrinsic).get();
							const Pose3 pose_J = sfm_data_.GetPoseOrDie(view_J);
							const Vec2 xJ = features_provider_->feats_per_view.at(J)[allViews_of_track.at(J)].coords().cast<double>();
								...
								)
							{
								...
								view_J->features[trackId] = allViews_of_track.at(J);
			...
			sfm_data_.views.at(viewIndex)->is_estimated = true;
			double dTimeF = timer.elapsedMs();
			std::cout	<< "Resection Task done.prepare data timeA:" << dTimeA
						<< "timeB:" << dTimeB -dTimeA
						<< "timeC:" << dTimeC - dTimeB
						<< "timeD:" << dTimeD - dTimeC
						<< "timeF:" << dTimeF - dTimeD
						<< "total time(ms): " << timer.elapsedMs() << std::endl;
			return true;
		}
		...
		bool SequentialSfMReconstructionEngine::BundleAdjustment()
		{
			...
			bool bSuccess = bundle_adjustment_obj.Adjust(sfm_data_, ba_refine_options);
			std::cout << "BundleAdjustment Task done in (ms): " << timer.elapsedMs() << std::endl;
			return bSuccess;
		}
		bool SequentialSfMReconstructionEngine::PartialBundleAdjustment()
		{
			system::Timer timer;
			Bundle_Adjustment_Ceres::BA_Ceres_options options;
			if (ceres::IsSparseLinearAlgebraLibraryTypeAvailable(ceres::SUITE_SPARSE) ||
			...
			bool bSuccess = bundle_adjustment_obj.PartialAdjust(sfm_data_, reconstructed_views_, ba_refine_options);
			std::cout << "PartialBundleAdjustment Task done in (ms): " << timer.elapsedMs() << std::endl;
			return bSuccess;
		}
		/**
